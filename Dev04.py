...
PACKETS_IN_FRAME_REMAIN = BYTES_IN_FRAME % BYTES_IN_PACKET  # 368
...
class DCA1000:
    ...
    def rearrange_frames(self):
        ...
        for packet in self.packet_data_list:
            current_frame.extend(packet)
            if len(current_frame) >= BYTES_IN_FRAME:
                frames.append(current_frame[:BYTES_IN_FRAME])
                current_frame = current_frame[BYTES_IN_FRAME:]
