    ...
    with open(file_path, 'w') as file:
        ...
        while time.time() - start_time <=8:
            ...
            if adc_data is not None:
                ...
                # print("Raw frame data:", adc_data)
                frame = dca.organize(adc_data, ADC_PARAMS['chirps']*2, 4,ADC_PARAMS['samples'])
                frame_count+=1
                # Format the frame data for file
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
                formatted_frame = f"{timestamp} Frame {frame_count} : {frame}\n"
                
                # Write frame data to file
                file.write(formatted_frame)
